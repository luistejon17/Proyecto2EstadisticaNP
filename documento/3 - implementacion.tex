...
\paragraph{Impacto medido sobre el bootstrap.} Sobre un test bootstrap completo con $B=199$ y muestra normal estándar de tamaño $n=160$, el nuevo esquema para $S_n$ resulta $3.5\times$ más rápido que la versión anterior basada en grid+Brent ($406$ ms vs.\ $1419$ ms). Para $T_n$, la versión Walsh local es $4\times$ más rápida para $n=20$ ($53$ ms vs.\ $215$ ms) pero $\approx 7\times$ más lenta para $n=160$ ($2.2$ s vs.\ $0.3$ s); el costo extra compra un mínimo \emph{global} verificable en lugar de un mínimo local heurístico.
