...
\begin{itemize}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Remuestras bootstrap}: búsqueda sobre los $k=\max(64,4n)$ promedios de Walsh más cercanos a $\hat\theta_{\min}$ de la muestra original. El soporte bootstrap es $\{X_i,\,2\hat\theta_{\min}-X_i\}$, simétrico \emph{exactamente} en $\hat\theta_{\min}$, por lo que el argmin de cada remuestra está a distancia $\mathcal{O}(n^{-1/2})$ de este punto. Usar $\hat\theta_{\min}$ como pivote (en lugar de la mediana de la remuestra) permite reducir $k$ en un factor $2$ respecto a la fase~1 sin pérdida apreciable de precisión.
  \end{enumerate}
