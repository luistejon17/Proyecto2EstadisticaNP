...
\begin{itemize}
  \item $\hat\theta_{\min}$: minimizador de $T_n$ (resp.\ $S_n$). La Sección~\ref{sec:estim-theta} detalla los esquemas numéricos. En el bootstrap se aplica el \emph{mismo} procedimiento tanto a la muestra original como a cada remuestra: búsqueda de $\hat\theta_{\min}$ sobre los $k=8n$ promedios de Walsh más cercanos a la mediana muestral de la remuestra (pivote $\sqrt{n}$-consistente para el centro de cada resample). Usar un ancla fija $\hat\theta_{\min}$ de la muestra original en lugar de la mediana de cada remuestra es incorrecto: produce $\hat\theta^*$ subóptimo en $\approx 30\%$ de los casos, lo que infla $T_n^*$ y sesga el p-valor hacia 1. Con ancla adaptiva (mediana del resample) y $k=8n$ la tasa de error contra la búsqueda exhaustiva es $0\%$.
  Para $S_n$ con $q=2$, \texttt{L-BFGS-B} con gradiente analítico reemplaza la búsqueda Walsh y también se aplica de manera idéntica a cada remuestra. El caso $q=1$ se excluye del estimador argmin (véase la subsección siguiente).
