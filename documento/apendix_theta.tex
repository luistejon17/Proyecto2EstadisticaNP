...
Sin embargo, esta expresión no define un gradiente clásico en los puntos donde \(\sin((\Phi_n(t)-t\theta)/2)=0\). En consecuencia, el criterio para \(q=1\) puede presentar puntos de no diferenciabilidad. En la implementación final, este gradiente se usa con \texttt{L-BFGS-B} para la \emph{estimación puntual} sobre la muestra original (funciona bien en la práctica). Sin embargo, en el \emph{bootstrap} la discontinuidad del gradiente hace divergir la secante batcheada (el paso $\Delta\theta \propto 1/dg$ explota cuando los gradientes de dos iterados consecutivos casi no cambian), por lo que el bootstrap de argmin para $q=1$ usa el grid vectorizado \texttt{\_sn\_boot\_batch\_argmin}.
...
Este método redujo el tiempo de cómputo frente al bucle secuencial y se adoptó como implementación final para $q=2$ en el bootstrap. Para $q=1$ no se usa la secante, porque el gradiente discontinuo causa divergencia; en su lugar se usa el grid vectorizado.

\subsection*{Conclusión del apéndice}

Los intentos anteriores fueron útiles para entender la estructura numérica de los estimadores por argmin. La búsqueda local en promedios de Walsh mostró que el minimizador de \(T_n\) está fuertemente ligado a la geometría discreta de la muestra, pero fue superada por el algoritmo exacto de Schuster-Narvarte. El gradiente formal para \(q=1\) resultó útil: se usa con \texttt{L-BFGS-B} para la estimación puntual, aunque en el bootstrap su discontinuidad obliga a recurrir al grid vectorizado. La secante batcheada se adoptó como método final para el bootstrap de argmin con $q=2$, donde el gradiente es continuo y la convergencia es fiable.
